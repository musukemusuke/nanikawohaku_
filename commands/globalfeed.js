...
module.exports = {
    ...
    async execute(interaction) {
        ...
        db.all(`SELECT * FROM posts WHERE (is_private = 0 OR (is_private = 1 AND (allowed_users LIKE ? OR author_username = ?))) ORDER BY created_at DESC LIMIT 50`, [`%${interaction.user.username}%`, interaction.user.username], async (err, rows) => {
            if (err) {
